...
class MyLoansFragment : Fragment() {
    ...
    private fun actualizarInterfaz(userId: Int) {
        val loans = if (userId != -1) dbHandler.getPrestamosPorUsuario(userId) else emptyList()
        val remaining = 3 - loans.size
        binding.tvRemainingLoans.text = "Préstamos restantes: $remaining"
        
        adapter.updateList(loans)
        binding.tvEmptyState.visibility = if (loans.isEmpty()) View.VISIBLE else View.GONE
    }
